...
func SetupUserSwagger(api *swag.API) {
	...
	api.AddEndpoint(
		endpoint.New(
			http.MethodPost, "/users/{id}/upload-image",
			endpoint.Handler(userController.UploadProfileImage),
			endpoint.Summary("Upload a profile image"),
			endpoint.Description("Allows a user to upload their profile image"),
			endpoint.Path("id", "string", "ID of the user uploading the image", true),
