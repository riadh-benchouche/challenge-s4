...
func SetupAssociationSwagger(api *swag.API) {
	...
	api.AddEndpoint(
		endpoint.New(
			http.MethodPost, "/associations/{id}/upload-image",
			endpoint.Handler(associationController.UploadProfileImage),
			endpoint.Summary("Upload profile image"),
			endpoint.Description("Uploads an image for the specified association"),
			endpoint.Path("id", "string", "ID of the association", true),
			endpoint.Response(http.StatusOK, "Image uploaded successfully", endpoint.SchemaResponseOption(map[string]string{
				"message":   "Image uploaded successfully",
				"image_url": "https://example.com/image.png",
			})),
			endpoint.Response(http.StatusBadRequest, "Invalid image file"),
